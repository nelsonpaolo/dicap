$(document).ready(function() {
    ...
    function clickBoton() {
        ...
        if(valido) {
            $.get(ajaxUrlIngresoAlta, {codigo: cadena}, function(data) {
                if(data.trim() === 'ok') {
                    bootbox.confirm('Cuando los datos del Ingreso se registren NO prodrán ser <strong>Editados</strong> o <strong>Eliminados</strong> <br> ¿Desea proceder con el Registro del Ingreso?', function(result) {
                        if(result == true){
                            $('#f').submit();
                        }
                    });
